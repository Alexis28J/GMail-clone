...
//:boolean indica che il metodo restituisce un valore booleano (vero o falso).
   const regex =
     /^(?=.*[a-z])      // minuscola
      (?=.*[A-Z])       // maiuscola
      (?=.*\d)          // numero
   ...
   // Il metodo test() verifica se la password corrisponde al pattern definito dalla regex e restituisce true o false.
 }
 ```
...
### Modifica: 
Ho sostituito la validazione dell'username con una più restrittiva, che richiede almeno 3 caratteri.
