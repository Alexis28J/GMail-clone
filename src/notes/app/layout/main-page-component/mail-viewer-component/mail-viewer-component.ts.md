...
Il componente accetta un `input email` di tipo `EmailInterface` o `null`, che riceve le informazioni dell'email selezionata dal componente genitore, e fornisce due `output`, `reply` e `forward`, che emettono eventi quando l'utente decide di rispondere o inoltrare l'email. 

I metodi `onReply` e `onForward` verificano se l'email è presente prima di emettere gli eventi corrispondenti.
